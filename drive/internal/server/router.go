...
func NewRouter(deps Dependencies) *gin.Engine {
	...
	if deps.AuthService != nil {
		...
		{
			presignedService := presigned.NewService(deps.ObjectStore, deps.Config.PresignedTTL)

			presignedHandler := presigned.NewHandler(
				presignedService,
				deps.BucketService,
				deps.FileService,
			)

			presignedHandler.RegisterRoutes(protected)
		}
	}
